...
public class TcpMarketPublishClient implements MarketPublishClient {
  ...
  public void conn(String host, int port, Consumer<Message<?>> consumer, Handler<AsyncResult<NetSocket>> connectHandler) {
      ...
      vertx.createNetClient(options)
           .connect(port, host, ar -> {
             if (ar.succeeded()) {
               ...
               final FrameParser parser = new FrameParser(connAr -> {
                 if (connAr.succeeded()) {
                   if (this.consumer != null) {
                     this.consumer.accept(connAr.result());
                   }
                 }
               });
    ...
    Buffer emptyBuffer = Buffer.buffer(5);
    emptyBuffer.appendInt(1);
    emptyBuffer.appendByte((byte) 0);
...
//    });
  ...
  public void send(byte[] binMsg) {
    if (this.client != null) {
      this.client.write(Buffer.buffer(binMsg), ar -> {
        if (ar.succeeded()) {
          if (this.client.writeQueueFull()) {
            this.client.pause();
            this.client.drainHandler(done -> this.client.resume());
  ...
  public void send(Buffer bufferMsg) {
    if (this.client != null) {
      this.client.write(bufferMsg,ar -> {
        if (ar.succeeded()) {
          if (this.client.writeQueueFull()) {
            this.client.pause();
            this.client.drainHandler(done -> this.client.resume());
